...
def cliente():
    ...
    url = "http://10.100.100.13:8081/query/objects.json?QC&last=%s" % (time.strftime("%Y%m%d.%H%M%S", time.localtime()))
    #print url
    latencia = dict()
    try:
...
def verificar(latencia):
    ...
    estacion.execute("select valor,fecha,estacion_id from service_estado13 where servidor='013' order by estacion_id")
        ...
        for i in latencia:
            ...
            for j in cursor_estado:
                if data['id_estacion']==j[2]:
                    cursor.execute("UPDATE service_estado13 SET valor='%s',fecha='%s' WHERE estacion_id=%s and servidor='013';" %(cambio(estado(data['valor']),j[0]),data['fecha'],data['id_estacion']))
    else:
        cursor.execute('''delete from service_estado13''')
        conn.commit()
        for i in latencia:
            data = latencia[i]
            cursor.execute('''INSERT INTO service_estado13 (valor,fecha,estacion_id,servidor) VALUES(?,?,?,?)''',(estado(float(data['valor'])), data['fecha'],data['id_estacion'],'013'))
    conn.commit()
    conn.close()
    estados()
'''
def funcionamiento(latencia):
    ...
    for i in latencia:
        data = latencia[i]
        cursor.execute("INSERT INTO service_funcionamiento_temp (valor,fecha,estacion_id,servidor) VALUES(%s,'%s',%s,'013');" % (data['disp'], datetime.now(), data['id_estacion']))
    conn.commit()
    conn.close()
...
def estados():
    ...
    estado = conn.cursor()
    cursor = conn.cursor()
    print('\nMonitor13! Time: %s' % datetime.now())
    estado.execute("select estacion, valor, estacion_id, fecha from service_estado13 sl inner join  service_estacion se on se.id=sl.estacion_id and servidor='013' order by estacion_id")
    ...
    for i in cursor_latencia:
        if i[1] == "entra":
            ...
            cursor.execute('''INSERT INTO service_historial (valor,fecha,estacion_id,servidor) VALUES (?,?,?,'013')''',('in',datetime.now(), i[2]))
            conn.commit()

        elif i[1] == "salio":
            print "Estacion %s Sale %s" % (i[0],i[3])
            cursor.execute('''INSERT INTO service_historial (valor,fecha,estacion_id,servidor) VALUES (?,?,?,'013')''',('out',datetime.now(), i[2]))
            conn.commit()
